...
func main() {
	...
	floor := physics.NewBodyRectangle(vector2.MakeFloat32(float32(screenWidth)/2, float32(screenHeight)), float32(screenHeight), 100, 10)
	floor.Enabled = false // Disable body state to convert it to static (no dynamics, but collisions)
	wall := physics.NewBodyRectangle(vector2.MakeFloat32(float32(screenWidth)/2, float32(screenHeight)*0.8), 10, 80, 10)
	wall.Enabled = false // Disable body state to convert it to static (no dynamics, but collisions)

	// Create left ramp physics body
	rectLeft := physics.NewBodyRectangle(vector2.MakeFloat32(25, float32(screenHeight)-5), 250, 250, 10)
	rectLeft.Enabled = false // Disable body state to convert it to static (no dynamics, but collisions)
	rectLeft.SetRotation(30 * rl.Deg2rad)

	// Create right ramp  physics body
	rectRight := physics.NewBodyRectangle(vector2.MakeFloat32(float32(screenWidth)-25, float32(screenHeight)-5), 250, 250, 10)
	rectRight.Enabled = false // Disable body state to convert it to static (no dynamics, but collisions)
	rectRight.SetRotation(330 * rl.Deg2rad)

	// Create dynamic physics bodies
	bodyA := physics.NewBodyRectangle(vector2.MakeFloat32(35, float32(screenHeight)*0.6), 40, 40, 10)
	bodyA.StaticFriction = 0.1
	bodyA.DynamicFriction = 0.1
	bodyA.SetRotation(30 * rl.Deg2rad)

	bodyB := physics.NewBodyRectangle(vector2.MakeFloat32(float32(screenWidth)-35, float32(screenHeight)*0.6), 40, 40, 10)
	...
	for !rl.WindowShouldClose() {
		...
		if rl.IsKeyPressed(rl.KeyR) { // Reset physics input
			...
			bodyA.Position = vector2.MakeFloat32(35, float32(screenHeight)*0.6)
			bodyA.Velocity = vector2.MakeFloat32(0, 0)
			bodyA.AngularVelocity = 0
			bodyA.SetRotation(30 * rl.Deg2rad)

			bodyB.Position = vector2.MakeFloat32(float32(screenWidth)-35, float32(screenHeight)*0.6)
			bodyB.Velocity = vector2.MakeFloat32(0, 0)
