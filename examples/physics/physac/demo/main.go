...
func main() {
	...
	floor := physics.NewBodyRectangle(vector2.MakeFloat32(float32(screenWidth)/2, float32(screenHeight)), 500, 100, 10)
	floor.Enabled = false // Disable body state to convert it to static (no dynamics, but collisions)

	// Create obstacle circle physics body
	circle := physics.NewBodyCircle(vector2.MakeFloat32(float32(screenWidth)/2, float32(screenHeight)/2), 45, 10)
