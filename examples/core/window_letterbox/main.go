...
func main() {
	...
	for !rl.WindowShouldClose() { // Detect window close button or ESC key
		...
		virtualMouse = virtualMouse.Clamp0V(vector2.MakeFloat32(gameScreenWidth, gameScreenHeight))
		...
		rl.DrawTexturePro(
			target.Texture,
			rect2.MakeFloat32(vector2.Zero[float32](), vector2.MakeFloat32(target.Texture.Width, -target.Texture.Height)),
			rect2.MakeFloat32(
				rl.GetScreenSize().ToFloat32().SubXY(float32(gameScreenWidth)*scale*0.5, float32(gameScreenHeight)*scale*0.5),
				vector2.MakeFloat32(float32(gameScreenWidth)*scale, float32(gameScreenHeight)*scale),
			),
